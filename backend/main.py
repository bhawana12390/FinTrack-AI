...
def convert_phonepe_to_transactions(phonepe_txns: List[PhonePeTxn]) -> List[Transaction]:
    ...
    for txn in phonepe_txns:
        ...
        if txn.kind in ["CREDIT", "Received", "Refund"]:
            kind = "CREDIT"
        elif "Received from" in txn.payee:
            kind = "CREDIT"
        elif "Refund" in txn.payee:
            kind = "CREDIT"
        elif txn.kind in ["DEBIT", "Paid"]:
            kind = "DEBIT"
        elif "Paid to" in txn.payee or "Payment to" in txn.payee:
            kind = "DEBIT"
